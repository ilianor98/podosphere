...
class GreekStandingsPage extends StatefulWidget {
  @override
  _GreekStandingsPageState createState() => _GreekStandingsPageState();
}

class _GreekStandingsPageState extends State<GreekStandingsPage> {
  ...
  Future<void> fetchStandingsData() async {
    try {
      final response = await http.get(
        Uri.parse('https://api-football-v1.p.rapidapi.com/v3/standings?season=2023&league=197'),
        headers: {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Greek Standings', style: TextStyle(color: Colors.white)),
